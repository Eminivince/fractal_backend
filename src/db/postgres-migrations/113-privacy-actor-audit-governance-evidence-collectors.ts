...
/** Adds actor-owned audit activity and uploader-owned governance-evidence metadata. */
export const privacyActorAuditGovernanceEvidenceCollectorsMigration: PostgresMigration = {
  version: "113-privacy-actor-audit-governance-evidence-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12_v13_v14_v15_v16_v17_v18_v19_v20_v21_v22_v23_v24_v25_v26_v27_v28_v29_v30_v31"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',
           blocker='The identity''s own immutable audit action, actor type, entity type, and occurrence time are available to access without organization or entity identifiers, reasons, payloads, hashes, sequence values, or other actors.'
     WHERE source_key='postgres.fractal.audit_events';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',portability_status='available',
           blocker='Uploader-owned governance-evidence kind, filename, MIME type, and creation time are available without organization or offering identifiers, binary content, sizes, hashes, storage locators, or other uploaders.'
     WHERE source_key='postgres.fractal.governance_evidence_documents';
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v31' THEN RAISE EXCEPTION 'privacy package preparation requires an active v31 content profile after actor-audit-governance-evidence collector activation'; END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 95 ELSE 23 END;
