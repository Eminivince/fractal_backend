...
/** Adds access-only lifecycle metadata for requester-linked governed object cleanup. */
export const privacyStorageCleanupCollectorMigration: PostgresMigration = {
  version: "122-privacy-storage-cleanup-collector",
  sql: `
    UPDATE fractal.platform_configuration_definitions SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12_v13_v14_v15_v16_v17_v18_v19_v20_v21_v22_v23_v24_v25_v26_v27_v28_v29_v30_v31_v32_v33_v34_v35_v36_v37"}'::jsonb WHERE configuration_key='privacy.rights.content_profile';
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='relational_identity',access_status='available',portability_status='unavailable',
           blocker='Access includes only lifecycle status and timestamps for governed support-attachment deletion tasks linked through a requester-owned case. Orphan cleanup, object keys, metadata and provider errors, worker claims, retry schedules, attempt counts, internal identifiers, other requesters, portability, and all rights execution remain excluded.'
     WHERE source_key='postgres.fractal.storage_cleanup_tasks';
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v37' THEN RAISE EXCEPTION 'privacy package preparation requires an active v37 content profile after governed-storage-cleanup collector activation'; END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 104 ELSE 24 END;
