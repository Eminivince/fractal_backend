...
/**
 * Adds access-only subject governance evidence under the v3 field catalogue.
 * Other administrators, internal reasons, key fingerprints, export filters,
 * and exported audit content are excluded before profile selection.
 */
export const privacyGovernanceEvidenceCollectorsMigration: PostgresMigration = {
  version: "085-privacy-governance-evidence-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical subject-linked governance metadata collection is implemented; portability excludes this platform-derived evidence, while correction, restriction, objection, erasure, complete-package delivery, and approved retention remain unavailable.'
     WHERE source_key IN (
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3')
    ...
    BEGIN
      ...
      IF NEW.request_type='access' AND NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v3' THEN
        RAISE EXCEPTION 'access preparation requires an active v3 content profile after governance collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 18 ELSE 8 END;
