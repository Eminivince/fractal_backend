...
/**
 * Adds requester-owned support case facts to access and portability, plus the
 * requester-visible event timeline to access. Internal notes, staff identity,
 * service evidence, notifications, attachments, holds, storage, and cleanup
 * state remain outside collection.
 */
export const privacySupportCaseCollectorsMigration: PostgresMigration = {
  version: "091-privacy-support-case-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           ...
           blocker='Canonical requester-owned support case collection is implemented for access and bounded portability without assignee identity, command keys, notification or provider diagnostics, service evidence, attachments, storage references, holds, disposition, or cleanup state; correction, restriction, objection, erasure, complete-package delivery, and approved retention remain unavailable.'
     WHERE source_key='postgres.fractal.support_cases';
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical requester-visible support timeline collection is implemented for access without internal notes, hidden workflow events, assignee or actor identity, command keys, notification or provider diagnostics, service evidence, attachments, storage references, holds, disposition, or cleanup state; portability and all execution rights remain unavailable.'
     WHERE source_key='postgres.fractal.support_case_events';
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6','privacy-safe-fields-v7','privacy-safe-fields-v8','privacy-safe-fields-v9')
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v9' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v9 content profile after support-case collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 33 ELSE 10 END;
