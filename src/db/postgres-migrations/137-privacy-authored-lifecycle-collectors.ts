...
/** Activates access-only collectors where the requester's authorship, receipt, or actor link is exact. */
export const privacyAuthoredLifecycleCollectorsMigration: PostgresMigration = {
  version: "137-privacy-authored-lifecycle-collectors",
  sql: `
    DO $$
    BEGIN
      IF EXISTS (
        SELECT 1 FROM fractal.idempotency_commands
        WHERE attribution_status<>'attributed' OR actor_identity_id IS NULL
      ) THEN
        RAISE EXCEPTION 'privacy idempotency collection requires approved removal or exact attribution of every legacy command';
      END IF;
    END; $$;
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',portability_status='unavailable',
           blocker='Access is limited to immutable actor-attributed route, response status, and lifecycle times. Scope keys, command keys, request hashes, response bodies, internal identifiers, and replay material are excluded. Migration fails closed while any legacy-unattributed command exists; portability and rights execution remain separately governed.'
     WHERE source_key='postgres.fractal.idempotency_commands';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='relational_identity',access_status='available',portability_status='unavailable',
           blocker='Access is limited to the authenticated submitter or reviewer, or to an exact frozen recipient of an approved notice. It excludes organization, offering, reservation, policy-version, command, reviewer/submitter identity, decision-reason, and internal identifiers. Notice content is returned only to its author/reviewer or intended recipient. Portability and rights execution remain separately governed.'
     WHERE source_key IN (
       'postgres.fractal.offering_notice_requests','postgres.fractal.offering_notices',
       'postgres.fractal.offering_notice_recipients','postgres.fractal.offering_notice_recipient_events'
     );
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='relational_identity',access_status='available',portability_status='unavailable',
           blocker='Access is limited to the authenticated creator, uploader, archiver, accessor, disposition requester, or disposition reviewer. Document binaries, storage keys, organization and document identifiers, titles, references, archive and internal event reasons, policy IDs/hashes, command keys, other actors, and unrelated organization records are excluded before profile projection. Portability and rights execution remain separately governed.'
     WHERE source_key IN (
       'postgres.fractal.organization_documents','postgres.fractal.organization_document_versions',
       'postgres.fractal.organization_document_events','postgres.fractal.organization_document_access_events',
       'postgres.fractal.organization_document_disposition_requests','postgres.fractal.organization_document_dispositions'
     );
    ...
    BEGIN
      ...
      END IF;
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v42' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v42 content profile after authored-lifecycle collector activation';
      END IF;
      expected_rule_count:=CASE NEW.request_type WHEN 'access' THEN 138 ELSE 24 END;
