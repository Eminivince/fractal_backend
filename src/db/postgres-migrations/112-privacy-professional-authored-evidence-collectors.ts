...
/** Adds direct-identity professional declarations, activity, and uploader-owned evidence metadata. */
export const privacyProfessionalAuthoredEvidenceCollectorsMigration: PostgresMigration = {
  version: "112-privacy-professional-authored-evidence-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12_v13_v14_v15_v16_v17_v18_v19_v20_v21_v22_v23_v24_v25_v26_v27_v28_v29_v30"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',portability_status='available',
           blocker='The identity own conflict declaration and authored notes are available with the public work-order reference and declaration time, without mandate content, counterpart identities, decisions, or internal identifiers.'
     WHERE source_key='postgres.fractal.professional_work_order_conflicts';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',
           blocker='The identity own professional work-order activity type and time are available to access without reasons, payloads, internal identifiers, other actors, confidential mandate content, or portability.'
     WHERE source_key='postgres.fractal.professional_work_order_events';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',portability_status='available',
           blocker='Uploader-owned finance-exception evidence type, filename, MIME type, and upload time are available without binary content, hashes, storage locators, case internals, resolution material, or other uploaders.'
     WHERE source_key='postgres.fractal.professional_finance_exception_evidence';
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v30' THEN RAISE EXCEPTION 'privacy package preparation requires an active v30 content profile after professional-authored-evidence collector activation'; END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 93 ELSE 22 END;
