...
/** Adds identity-linked allocation and chain-execution lifecycle facts to access only. */
export const privacyInvestmentAllocationCollectorsMigration: PostgresMigration = {
  version: "096-privacy-investment-allocation-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12_v13_v14"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources SET subject_linkage='direct_identity',access_status='available',
      blocker='Canonical identity-owned allocation lifecycle facts are implemented for access without compliance snapshots, policy hashes, decision reasons, maker/checker identities, organization/internal identifiers, or settlement/title claims; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
      WHERE source_key='postgres.fractal.investment_allocation_requests';
    UPDATE fractal.privacy_data_sources SET subject_linkage='relational_identity',access_status='available',
      blocker='Canonical identity-linked allocation chain-operation facts are implemented for access without failure reasons, organization/internal identifiers, dispatch internals, or finality/title claims; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
      WHERE source_key='postgres.fractal.investment_allocation_chain_operations';
    UPDATE fractal.privacy_data_sources SET subject_linkage='relational_identity',access_status='available',
      blocker='Canonical identity-linked allocation dispatch outcome metadata is implemented for access without worker identities, failure reasons, lease/internal identifiers, or finality/title claims; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
      WHERE source_key='postgres.fractal.investment_allocation_chain_dispatch_claims';
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_version_number>0 AND content_profile_projection_version>0
          AND content_profile_value_sha256 ~ '^[0-9a-f]{64}$'
          AND length(content_profile_reference) BETWEEN 3 AND 120 AND length(content_profile_name) BETWEEN 10 AND 160
          AND content_profile_schema_version='privacy-content-profile-v1'
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6','privacy-safe-fields-v7','privacy-safe-fields-v8','privacy-safe-fields-v9','privacy-safe-fields-v10','privacy-safe-fields-v11','privacy-safe-fields-v12','privacy-safe-fields-v13','privacy-safe-fields-v14')
    ...
    BEGIN
      ...
      SELECT version.version_number,version.value_sha256,version.proposed_value,projection.projection_version INTO exact_profile
        ...
        JOIN fractal.platform_configuration_versions version ON version.id=projection.active_version_id AND version.status='active'
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v14' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v14 content profile after investment-allocation collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 46 ELSE 12 END;
