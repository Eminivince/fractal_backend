...
/** Subject-bounded access collectors for approved distribution privacy-treatment evidence. */
export const privacyDistributionTreatmentCollectorsMigration: PostgresMigration = {
  version: "134-privacy-distribution-treatment-collectors",
  sql: `
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Approved requester-visible treatment evidence is available through the subject-bounded v40 access collector. Portability, complete distribution coverage, production delivery, and physical disposition remain unavailable.'
     WHERE source_key IN(
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v40' THEN RAISE EXCEPTION 'privacy package preparation requires an active v40 content profile after distribution treatment collector activation'; END IF;
      expected_rule_count:=CASE NEW.request_type WHEN 'access' THEN 125 ELSE 24 END;
