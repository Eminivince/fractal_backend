...
/**
 * Adds access-only subject metadata for prior privacy package preparations.
 * Collection runs before the new preparation is inserted, so the current row
 * cannot recursively include itself. Internal manifests, hashes, snapshots,
 * audit watermarks, command keys, and administrator identities stay excluded.
 */
export const privacyPackagePreparationCollectorMigration: PostgresMigration = {
  version: "088-privacy-package-preparation-collector",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical access collection now includes subject-linked metadata for prior incomplete package preparations without manifests, hashes, snapshots, audit watermarks, command keys, or administrator identities; portability, correction, restriction, objection, erasure, complete-package delivery, and approved retention remain unavailable.'
     WHERE source_key='postgres.fractal.privacy_rights_package_preparations';
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6')
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v6' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v6 content profile after package-preparation collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 25 ELSE 9 END;
