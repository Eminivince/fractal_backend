...
/**
 * Adds access-only identity-verification lifecycle evidence under the v4 field
 * catalogue. Provider identifiers, worker state, errors, rejection labels, and
 * callback integrity material are excluded before profile selection.
 */
export const privacyIdentityVerificationCollectorsMigration: PostgresMigration = {
  version: "086-privacy-identity-verification-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical subject-linked identity-verification lifecycle metadata collection is implemented; provider identifiers, worker state, retry errors, rejection labels, and integrity material are excluded, the provider copy remains unresolved, and portability excludes this derived evidence.'
     WHERE source_key IN (
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4')
    ...
    BEGIN
      ...
      IF NEW.request_type='access' AND NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v4' THEN
        RAISE EXCEPTION 'access preparation requires an active v4 content profile after identity-verification collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 20 ELSE 8 END;
