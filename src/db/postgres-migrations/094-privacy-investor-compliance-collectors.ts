...
/**
 * Adds identity-owned compliance profile and review lifecycle metadata to
 * access. Provider evidence, reviewer identity, deliberation, identifiers,
 * snapshots, and portability remain excluded.
 */
export const privacyInvestorComplianceCollectorsMigration: PostgresMigration = {
  version: "094-privacy-investor-compliance-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           ...
           blocker='Canonical identity-owned compliance approval lifecycle metadata is implemented for access without profile snapshots, embedded provider evidence, reviewer identity, organization/internal identifiers, or deliberation; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
     WHERE source_key='postgres.fractal.investor_compliance_profile_reviews';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           access_status='available',
           blocker='Canonical current identity-owned compliance assessment metadata is implemented for access without embedded evidence, provider identifiers/hashes, or internal identifiers; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
     WHERE source_key='postgres.fractal.investor_compliance_profiles';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           access_status='available',
           blocker='Canonical identity-owned compliance review-request lifecycle metadata is implemented for access without embedded provider evidence, decision reasons, maker/checker identities, organization/internal identifiers, or deliberation; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
     WHERE source_key='postgres.fractal.investor_compliance_review_requests';
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6','privacy-safe-fields-v7','privacy-safe-fields-v8','privacy-safe-fields-v9','privacy-safe-fields-v10','privacy-safe-fields-v11','privacy-safe-fields-v12')
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v12' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v12 content profile after investor-compliance collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 40 ELSE 11 END;
