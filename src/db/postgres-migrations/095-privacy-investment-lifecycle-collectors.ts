...
/**
 * Adds identity-owned eligibility, commitment, and reservation lifecycle facts
 * to access, plus bounded requester-initiated reservation data to portability.
 * Internal identifiers, policy/evidence snapshots, command keys, provider
 * records, and accounting records remain excluded.
 */
export const privacyInvestmentLifecycleCollectorsMigration: PostgresMigration = {
  version: "095-privacy-investment-lifecycle-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12_v13"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           ...
           blocker='Canonical identity-owned offering eligibility outcomes are implemented for access without policy/evidence snapshots, internal identifiers, organization identifiers, provider material, or deliberation; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
     WHERE source_key='postgres.fractal.investment_eligibility_snapshots';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           access_status='available',
           blocker='Canonical identity-owned investment commitment lifecycle facts are implemented for access without internal identifiers, organization identifiers, payment-provider records, receipts, reconciliation cases, journal records, or settlement claims; portability and all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
     WHERE source_key='postgres.fractal.investment_commitments';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',
           access_status='available',
           portability_status='available',
           blocker='Canonical identity-owned reservation lifecycle facts are implemented for access and bounded portability without internal identifiers, command keys, linked eligibility/agreement/commitment identifiers, provider records, accounting records, or settlement claims; all correction, restriction, objection, erasure, delivery, and approved-retention operations remain unavailable.'
     WHERE source_key='postgres.fractal.investment_reservations';
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6','privacy-safe-fields-v7','privacy-safe-fields-v8','privacy-safe-fields-v9','privacy-safe-fields-v10','privacy-safe-fields-v11','privacy-safe-fields-v12','privacy-safe-fields-v13')
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v13' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v13 content profile after investment-lifecycle collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 43 ELSE 12 END;
