...
/** Adds direct-identity professional payout verification and finance-governance participation metadata. */
export const privacyProfessionalFinanceParticipationCollectorsMigration: PostgresMigration = {
  version: "114-privacy-professional-finance-participation-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12_v13_v14_v15_v16_v17_v18_v19_v20_v21_v22_v23_v24_v25_v26_v27_v28_v29_v30_v31_v32"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',
           blocker='The identity''s own payout-profile verification role, version, rail, currency, status, and lifecycle times are available to access without account-holder or account-number data, provider references, organization data, internal identifiers, or other actors.'
     WHERE source_key='postgres.fractal.professional_payout_profile_versions';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',
           blocker='The identity''s own tax-treatment preparation or approval role, version, status, and lifecycle times are available to access without organization data, jurisdictions, service classes, tax rates, legal sources, policy dates, internal identifiers, or other actors.'
     WHERE source_key='postgres.fractal.professional_invoice_tax_treatments';
    UPDATE fractal.privacy_data_sources
       SET subject_linkage='direct_identity',access_status='available',
           blocker='The identity''s own finance-policy preparation or approval role, version, status, and lifecycle times are available to access without organization data, resolution types, currencies, financial limits, policy references or dates, internal identifiers, or other actors.'
     WHERE source_key='postgres.fractal.professional_finance_approval_policies';
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v32' THEN RAISE EXCEPTION 'privacy package preparation requires an active v32 content profile after professional-finance-participation collector activation'; END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 98 ELSE 23 END;
