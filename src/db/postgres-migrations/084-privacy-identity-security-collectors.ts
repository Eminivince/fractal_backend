...
/**
 * Adds access-only, secret-free identity/security collectors under the v2
 * field catalogue. Existing v1 preparation evidence remains immutable and
 * readable; new access preparation fails closed until v2 is independently
 * approved and active.
 */
export const privacyIdentitySecurityCollectorsMigration: PostgresMigration = {
  version: "084-privacy-identity-security-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical secret-free access collection is implemented; portability is not applicable to this derived security metadata, while correction, restriction, objection, erasure, complete-package delivery, and approved retention remain unavailable.'
     WHERE source_key IN (
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2')
    ...
    BEGIN
      ...
      IF NEW.request_type='access' AND NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v2' THEN
        RAISE EXCEPTION 'access preparation requires an active v2 content profile after identity-security collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 13 ELSE 8 END;
