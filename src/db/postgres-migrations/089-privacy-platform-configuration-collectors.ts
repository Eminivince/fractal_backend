...
/**
 * Adds access-only actor-linked platform-configuration lifecycle evidence.
 * Configuration values, hashes, validation and impact material, rationales,
 * event evidence, failure detail, and other actors remain outside collection.
 */
export const privacyPlatformConfigurationCollectorsMigration: PostgresMigration = {
  version: "089-privacy-platform-configuration-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical actor-linked platform-configuration lifecycle collection is implemented for access without configuration values, hashes, validation or impact material, rationales, event evidence, failure detail, or other actors; portability, correction, restriction, objection, erasure, complete-package delivery, and approved retention remain unavailable.'
     WHERE source_key IN (
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6','privacy-safe-fields-v7')
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v7' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v7 content profile after platform-configuration collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 29 ELSE 9 END;
