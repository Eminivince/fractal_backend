...
/**
 * Adds access-only metadata for requester-visible support attachments and their
 * integrity-verified access events. Binary content, internal attachments,
 * storage locators, actor identities, provider records, and lifecycle commands
 * remain outside collection.
 */
export const privacySupportAttachmentCollectorsMigration: PostgresMigration = {
  version: "092-privacy-support-attachment-collectors",
  sql: `
    UPDATE fractal.platform_configuration_definitions
       SET validation_schema='{"type":"object","required":["profileReference","profileName","schemaVersion","fieldCatalogVersion","jurisdictionCode","legalBasisReference","effectiveScope","access","portability"],"operationalValidator":"privacy_content_profile_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10"}'::jsonb
    ...
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical requester-visible support attachment metadata collection is implemented for access without binary content, internal attachments, uploader identity, command keys, storage locators, configuration hashes or identifiers, provider diagnostics, hold/disposition state, or cleanup evidence; portability, complete object-version retrieval, correction, restriction, objection, erasure, delivery, and approved retention remain unavailable.'
     WHERE source_key='postgres.fractal.support_case_attachments';
    UPDATE fractal.privacy_data_sources
       SET access_status='available',
           blocker='Canonical integrity-verified access-event collection is implemented for requester-visible support attachments without actor identity, internal attachments, storage locators, provider diagnostics, or lifecycle command material; portability, complete binary retrieval, correction, restriction, objection, erasure, delivery, and approved retention remain unavailable.'
     WHERE source_key='postgres.fractal.support_case_attachment_access_events';
    ...
    ALTER TABLE fractal.privacy_rights_package_preparations
      ...
      ADD CONSTRAINT privacy_package_content_profile_shape CHECK (
        ...
        (content_profile_binding_status='governed'
          ...
          AND content_profile_field_catalog_version IN ('privacy-safe-fields-v1','privacy-safe-fields-v2','privacy-safe-fields-v3','privacy-safe-fields-v4','privacy-safe-fields-v5','privacy-safe-fields-v6','privacy-safe-fields-v7','privacy-safe-fields-v8','privacy-safe-fields-v9','privacy-safe-fields-v10')
    ...
    BEGIN
      ...
      IF NEW.content_profile_field_catalog_version<>'privacy-safe-fields-v10' THEN
        RAISE EXCEPTION 'privacy package preparation requires an active v10 content profile after support-attachment collector activation';
      END IF;
      expected_rule_count := CASE NEW.request_type WHEN 'access' THEN 35 ELSE 10 END;
